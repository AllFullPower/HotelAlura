...
public class JFrameRegistrarCliente extends JFrame {
	...
	private JPanel btnBackSistemaReserva;
	...
	private List <JFormattedTextField> inputs = new ArrayList<>();
	 ...
	 * Create the frame.
	 */
	public JFrameRegistrarCliente(JFrameRegistrarReserva frameRegistrarReserva) {
		this.frameRegistrarReserva = frameRegistrarReserva;
	...
	public void constructorEscena() {
		...
		lblNewLabel = new JLabel("Registar Cliente");
		lblNewLabel.setBounds(74, 11, 194, 55);
		...
		comboBoxMetodoDePago.setModel(new DefaultComboBoxModel(new NacionalidadNumeroController().leerArchivoNacionalidad()));
		comboBoxMetodoDePago.setBounds(42, 225, 227, 30);
		panelnputsContainer.add(comboBoxMetodoDePago);
		
		btnBackSistemaReserva = new JPanel();
		btnBackSistemaReserva.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		btnBackSistemaReserva.setLayout(null);
		btnBackSistemaReserva.setBackground(new Color(244, 186, 51));
		btnBackSistemaReserva.setBounds(10, 413, 86, 30);
		panelnputsContainer.add(btnBackSistemaReserva);
		
		lblBack = new JLabel("Back");
		lblBack.setFont(new Font("Roboto", Font.PLAIN, 17));
		lblBack.setBounds(22, 8, 54, 14);
		btnBackSistemaReserva.add(lblBack);
		...
		listaDateChoosers.add(chooserFechaDeNacimiento);
		inputs.add(inputNombre);
		inputs.add(inputApellido);
		inputs.add(inputTelefono);
		...
		btnExit.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(JOptionPane.showConfirmDialog(null, "Seguro que quieres salir, perderas tu reserva?") == 0) {
					reservasDAO.elliminar(reservasController.getUltimoElementoRegistradoId());
					System.exit(0);
				};
			}
		});
		
		//----------BOTON BACK TO SISTEMA RESERVA
		
		btnBackSistemaReserva.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				setVisible(false);
			...
			public void mouseEntered(MouseEvent e) {
				...
				btnBackSistemaReserva.setBackground(new Color(223, 160, 13));
			}
			@Override
			public void mouseExited(MouseEvent e) {
		...
		btnSaveReservaCliente.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if(!inputsController.validarTxt(inputs) || !inputsController.validarDates(listaDateChoosers)) {
					JOptionPane.showMessageDialog(null, "No puedes dejar informacion vacia");
					return;
				}else {
					...
					Cliente cliente = new Cliente(nombre, apellido, fechaDeNacimiento, nacionalidad, numero);
